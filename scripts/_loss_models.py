...
def cosine(network, snapshots):
    ...
    for branch in passive_branches.index:
        ...
        for sn in snapshots:
            ...
            if passive_branches.at[branch, "s_nom_extendable"]:
                ...
                rhs = LExpression([(s_max_pu, network.model.passive_branch_s_nom[bt,bn]),(-1, network.model.loss[bt,bn,sn])])
    ...
    l_constraint(network.model, "lower_flow", lower_flow, list(passive_branches.index), snapshots)

    l_constraint(network.model, "upper_flow", upper_flow, list(passive_branches.index), snapshots)
